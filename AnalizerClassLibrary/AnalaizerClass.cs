        ...
        public static bool CheckCurrency()
        {
            ...
            for (int i = 0; i < expression.Length; i++)
            {
                ...
                else
                {
                    if (expression[i] == ')')
                    {
                        if (openBracket.Count ==
                            0) // варіант, при якому закриваюча дужка, використана без відкриваючої дужки                                                    
                        {
                            erposition = i;
                            if (ShowMessage)
                                MessageBox.Show
            ...
            for (int i = 0; i < expression.Length; i++)
            {
                ...
                if (char.IsDigit(currentSymbol))
                {
                    if (i < expression.Length - 1)
                    {
                        ...
                        if (nextSymbol == SYMBOL_OPEN_BRACKET || nextSymbol == SYMBOL_UNARY_MINUS ||
                            nextSymbol == SYMBOL_UNARY_PLUS)
                        {
                ...
                if (_operators.Contains(currentSymbol))
                {
                    if (i < expression.Length - 1)
                    {
                        ...
                        if (!char.IsDigit(nextSymbol) && nextSymbol != SYMBOL_OPEN_BRACKET &&
                            nextSymbol != SYMBOL_UNARY_MINUS && nextSymbol != SYMBOL_UNARY_PLUS)
                            return "&" + ErrorsExpression.ERROR_03;
                    }
                }
                ...
                if (_unary_operators.Contains(currentSymbol))
                {
                    ...
                    {
                        char nextSymbol = expression[i + 1];
                        if (nextSymbol == SYMBOL_CLOSE_BRACKET || _unary_operators.Contains(nextSymbol) ||
                            _operators.Contains(nextSymbol))
                            return "&" + ErrorsExpression.ERROR_03;
                    }
                    else
                        return "&" + ErrorsExpression.ERROR_05;
            ...
            for (int i = 0; i < res.Length; i++)
            {
                ...
                if (currentSymbol == SYMBOL_OPERATOR_ADD)
                {
                    ...
                    if (i < res.Length - 1)
                    {
                        ...
                        if ((nextSymbol == SYMBOL_OPEN_BRACKET || char.IsDigit(nextSymbol)) &&
                            (_operators.Contains(previosSymbol) || previosSymbol == SYMBOL_OPEN_BRACKET))
